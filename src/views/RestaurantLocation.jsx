...
const RestaurantLocation = (props) => {
  ...
  const onRegionChange = async(value) => {
    ...
    console.log('https://maps.googleapis.com/maps/api/geocode/json?address=' + value.latitude + ',' + value.longitude + '&key=' + GOOGLE_KEY)
  ...
  const check_phone = async() => {
    ...
    await axios({
      ...
      data:{ phone_with_code : props.restaurant_phone_with_code}
    })
    .then(async response => {
      ...
      if(response.data.result.is_available == 1){
       ...
       // phone_number = phone_number.replace("+"+phone_ref.current.getCountryCode(), "");
        navigation.navigate('Otp',{ data : response.data.result.otp, type: 2, phone_with_code : props.restaurant_phone_with_code, phone_number : props.restaurant_phone_number })
      }
    })
    .catch(error => {
      setLoading(false);
