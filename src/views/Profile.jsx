...
const Profile = (props) => {
  ...
  console.log('ID Image URI:', id_image);

  console.log(img_url+props.restaurant_profile_picture,'profile');
  ...
  const get_profile_update = async () => {
    ...
    await axios({
      ...
      data:{ id: global.id, email:props.restaurant_email, restaurant_name:props.restaurant_name,account_number:accountNumber }
    })
    .then(async response => {
      setLoading(false);
  ...
  const identityimageupdate = async(img_data) =>{
    await setLoading(true);
    RNFetchBlob.fetch('POST', api_url + 'restaurant/id_card_img', {
      'Content-Type' : 'multipart/form-data',
    }, [
      {  
        name : 'image',
      ...
      let data = await JSON.parse(resp.data);
      if(data.result){
       await identity_image_update(data.result);
        await setProfileTimer(false);
        await setTimeout(function(){setProfileTimer(true)}, 20000)
      }
    }).catch((err) => {
  ...
  const cheque_image_update = async (data) => {
    setLoading(true);
      await axios({
        method: 'post', 
        url: api_url+'restaurant/cancelled_cheque_img_update',
        data: {id:global.id, cancelled_cheque_img:data}
      })
      .then(async response => {
        setLoading(false);
        console.log(response)
  ...
  <SafeAreaView style={styles.container}>
    ...
    <View style={{  width:'100%',flexDirection:'row',height:160, backgroundColor:colors.theme_fg, position:'absolute', top:0,  borderBottomRightRadius:40}}>
    ...
    <TouchableOpacity onPress={select_photo.bind(this)} style={styles.box}>
      <View onPress={select_photo.bind(this)} style={styles.profile} >
        <Image style= {{ height: undefined,width: undefined, flex:1, borderRadius:50, borderWidth:3, borderColor: colors.theme_bg }} source={{ uri : img_url + props.restaurant_profile_picture}} />
      </View>
    </TouchableOpacity>
  </SafeAreaView>
);
