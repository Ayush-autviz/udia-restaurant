...
import { StyleSheet, View, Text, SafeAreaView, ScrollView, TouchableOpacity, Image, Linking, StatusBar, Platform } from 'react-native';
import Icon, { Icons } from '../components/Icons';
import * as colors from '../assets/css/Colors';
import { regular, veg, restaurant_order_detail, api_url, img_url, order_status_change, bold, cancel } from '../config/Constants';
import { useNavigation, useRoute } from '@react-navigation/native';
...
const OrderSummery = () => {
  ...
  const getOrderDetail = async () => {
    setLoading(true);
    try {
      const response = await axios.post(`${api_url}${restaurant_order_detail}`, {
        order_id: id,
      });
      setOrderDetails(response.data.result);
    } catch (error) {
  ...
  const getOrderStatusChange = async (slug, orderId) => {
    console.log({ order_id: orderId, slug });
   // setLoading(true);
   navigation.goBack();
    try {
      await axios.post(`${api_url}${order_status_change}`, {
        order_id: orderId,
        slug,
      });
     // setLoading(false);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={{ padding: 10 }} showsVerticalScrollIndicator={false}>
        <Loader visible={loading} />
        <View style={styles.header}>
          <TouchableOpacity onPress={navigation.goBack} style={{ width: '15%', justifyContent: 'center', alignItems: 'flex-start' }}>
            <Icon type={Icons.Ionicons} name="chevron-back-circle-outline" color={colors.theme_fg_two} style={{ fontSize: 35 }} />
          </TouchableOpacity>
          <View style={{ width: '75%', justifyContent: 'center', alignItems: 'flex-start' }}>
            <Text style={{ color: colors.theme_fg_two, fontFamily: bold, fontSize: 20 }}>Order Summary</Text>
          </View>
        </View>
        <Text style={{ color: colors.theme_fg_two, fontFamily: regular, fontSize: 16, letterSpacing: 1 }}>{orderDetails.customer_name}</Text>
        <View style={{ margin: 1 }} />
        <Text style={{ color: colors.grey, fontFamily: regular, fontSize: 10 }}>{orderDetails.address}</Text>
        <View style={{ margin: 5 }} />
        <View style={{ flexDirection: 'row', backgroundColor: colors.theme_bg_three, borderBottomWidth: 0.5, borderColor: colors.grey }}>
          <View style={{ width: '50%', justifyContent: 'center', alignItems: 'flex-start', paddingBottom: 10 }}>
            <Text style={{ fontFamily: regular, fontSize: 16, color: colors.theme_fg_two }}>Your Order</Text>
        ...
        {orderDetails.item_list.map((item) => (
          <View key={item.item_id} style={{ backgroundColor: colors.theme_bg_three, paddingBottom: 10, paddingTop: 10, borderBottomWidth: 0.5, borderColor: colors.grey, flexDirection: 'row' }}>
            <View style={{ width: '50%' }}>
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                <Image style={{ height: 15, width: 15 }} source={veg} />
                <View style={{ margin: 4 }} />
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
          ...
          <View style={{ width: '50%', alignItems: 'flex-end' }}>
            <Text style={{ fontSize: 14, color: colors.theme_fg_two, fontFamily: regular }}>{global.currency}{orderDetails.sub_total}</Text>
          </View>
        </View>
        <View style={{ margin: 5 }} />
        <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
          ...
          <View style={{ width: '50%', alignItems: 'flex-end' }}>
            <Text style={{ fontSize: 12, color: colors.grey, fontFamily: regular }}>{global.currency}{orderDetails.tax}</Text>
          </View>
        </View>
        <View style={{ margin: 5 }} />
        <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
          ...
          <View style={{ width: '50%', alignItems: 'flex-end' }}>
            <Text style={{ fontSize: 12, color: colors.grey, fontFamily: regular }}>{global.currency}{orderDetails.discount}</Text>
          </View>
        </View>
        <View style={{ margin: 5 }} />
        <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
          ...
          <View style={{ width: '50%', alignItems: 'flex-end' }}>
            <Text style={{ fontSize: 12, color: colors.grey, fontFamily: regular }}>{global.currency}{orderDetails.delivery_charge}</Text>
          </View>
        </View>
        <View style={{ margin: 5 }} />
        <View style={{ backgroundColor: colors.theme_bg_three, borderBottomWidth: 0.5, borderColor: colors.grey, flexDirection: 'row' }} />
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
          ...
          <View style={{ width: '50%', alignItems: 'flex-end' }}>
            <Text style={{ fontSize: 14, color: colors.theme_fg_two, fontFamily: regular }}>{global.currency}{orderDetails.total}</Text>
          </View>
        </View>
        <View style={{ position: 'absolute', top: '35%', left: '25%' }}>
          {allowedStatuses.includes(orderDetails.slug) && (
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
              <Image style={{ height: 125, width: 160 }} source={cancel} />
            </View>
          )}
        ...
        <View style={{ backgroundColor: colors.theme_bg_three, borderBottomWidth: 0.5, borderColor: colors.grey, flexDirection: 'row' }} />
        <View style={{ margin: 10 }} />
        <Text style={{ color: colors.theme_fg_two, fontFamily: regular, fontSize: 18 }}>
          Order details <Text style={{ fontFamily: bold, fontSize: 14, color: colors.theme_bg }}>[{orderDetails.status}]</Text>
        </Text>
        <View style={{ margin: 5 }} />
        <View style={{ flexDirection: 'row', backgroundColor: colors.theme_bg_three, borderBottomWidth: 0.5, borderColor: colors.grey }} />
        <View style={{ margin: 5 }} />
        <Text style={{ fontSize: 12, color: colors.grey, fontFamily: regular }}>Order Number</Text>
        <View style={{ margin: 1 }} />
        <Text style={{ fontSize: 14, color: colors.grey, fontFamily: regular }}>#{orderDetails.id}</Text>
        <View style={{ margin: 10 }} />
        <Text style={{ fontSize: 12, color: colors.grey, fontFamily: regular }}>Payment</Text>
        <View style={{ margin: 1 }} />
        <Text style={{ fontSize: 14, color: colors.grey, fontFamily: regular }}>{orderDetails.payment_name}</Text>
        <View style={{ margin: 10 }} />
        <Text style={{ fontSize: 12, color: colors.grey, fontFamily: regular }}>Date</Text>
        <View style={{ margin: 1 }} />
        <Text style={{ fontSize: 14, color: colors.grey, fontFamily: regular }}>{Moment(orderDetails.created_at).format('MMM DD, YYYY hh:mm A')}</Text>
        <View style={{ margin: 10 }} />
        <Text style={{ fontSize: 12, color: colors.grey, fontFamily: regular }}>Phone number</Text>
        <View style={{ margin: 1 }} />
        <Text style={{ fontSize: 14, color: colors.grey, fontFamily: regular }}>{orderDetails.phone_with_code}</Text>
        <View style={{ margin: 10 }} />
        <TouchableOpacity
          onPress={() => callCustomer(orderDetails.phone_with_code)}
          style={{ backgroundColor: colors.theme_bg_three, borderBottomWidth: 0.5, borderColor: colors.grey, borderTopWidth: 0.5, padding: 15, alignItems: 'center', justifyContent: 'center' }}
        >
          <Text style={{ fontFamily: regular, fontSize: 13, color: colors.red }}>Call {orderDetails.customer_name} ({orderDetails.phone_with_code})</Text>
        </TouchableOpacity>
        <View style={{ height: 40 }} />
      </ScrollView>
      {orderDetails.slug === 'restaurant_approved' && (
        <View style={{ left: 0, right: 0, bottom: 0, alignItems: 'center', height: 50, position: 'absolute', justifyContent: 'center' }}>
          <TouchableOpacity onPress={() => getOrderStatusChange('ready_to_dispatch', orderDetails.id)} style={styles.button}>
            <View style={{ width: '100%', alignItems: 'center', justifyContent: 'center' }}>
              <Text style={{ color: colors.theme_fg_three, fontFamily: bold, fontSize: 14 }}>Ready to Dispatch</Text>
            </View>
          </TouchableOpacity>
